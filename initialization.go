package xgp

import (
	"math/rand"

	"github.com/MaxHalford/xgp/op"
	"github.com/MaxHalford/xgp/tree"
)

// A Initializer generates a random Tree.
type Initializer interface {
	Apply(
		minHeight int,
		maxHeight int,
		newOp func(leaf bool, rng *rand.Rand) op.Operator,
		rng *rand.Rand,
	) tree.Tree
}

// FullInitializer generates Trees with node depths equal to Height.
type FullInitializer struct{}

// Apply FullInitializer.
func (init FullInitializer) Apply(
	minHeight int,
	maxHeight int,
	newOp func(leaf bool, rng *rand.Rand) op.Operator,
	rng *rand.Rand,
) tree.Tree {
	var (
		op = newOp(maxHeight == 0, rng)
		tr = tree.NewTree(op)
	)
	for i := 0; i < len(tr.Branches); i++ {
		br := init.Apply(0, maxHeight-1, newOp, rng)
		tr.Branches[i] = &br
	}
	return tr
}

// GrowInitializer generates Trees with node depths in [MinHeight, MaxHeight].
type GrowInitializer struct {
	PTerminal float64
}

// Apply GrowInitializer.
func (init GrowInitializer) Apply(
	minHeight int,
	maxHeight int,
	newOp func(leaf bool, rng *rand.Rand) op.Operator,
	rng *rand.Rand,
) tree.Tree {
	var (
		leaf = maxHeight == 0 || (minHeight <= 0 && rng.Float64() < init.PTerminal)
		op   = newOp(leaf, rng)
		tr   = tree.NewTree(op)
	)

	for i := 0; i < len(tr.Branches); i++ {
		br := init.Apply(minHeight-1, maxHeight-1, newOp, rng)
		tr.Branches[i] = &br
	}
	return tr
}

// RampedHaldAndHalfInitializer randomly applies GrowTreeInitializer or
// FullTreeInitializer.
type RampedHaldAndHalfInitializer struct {
	PFull           float64
	FullInitializer FullInitializer
	GrowInitializer GrowInitializer
}

// Apply RampedHaldAndHalfInitializer.
func (init RampedHaldAndHalfInitializer) Apply(
	minHeight int,
	maxHeight int,
	newOp func(leaf bool, rng *rand.Rand) op.Operator,
	rng *rand.Rand,
) tree.Tree {
	if rng.Float64() < init.PFull {
		var height = randInt(minHeight, maxHeight, rng)
		return init.FullInitializer.Apply(0, height, newOp, rng)
	}
	return init.GrowInitializer.Apply(minHeight, maxHeight, newOp, rng)
}
